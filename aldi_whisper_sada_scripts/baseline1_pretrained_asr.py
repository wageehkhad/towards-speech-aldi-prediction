...
"""
Baseline 1:
Audio -> pretrained Whisper ASR -> transcript -> Sentence-ALDi score.
"""
...
from submission_paths import CASABLANCA_DIR, MEDIASPEECH_DIR, RESULTS_DIR, SADA_DIR, path_str
...
def parse_args():
    ...
    ap.add_argument(
        ...
        help="Used for text/dialect metadata.",
    )
    ap.add_argument(
        "--casablanca-hf-dataset",
        default=path_str(CASABLANCA_DIR / "casablanca_audio_aldi"),
        help="load_from_disk() HF dataset used to fetch Casablanca audio arrays.",
    )
...
def main():
    ...
    _, metrics_sada = run_asr_sentence_aldi(
        ...
        progress_desc="Baseline1 [sada_test]",
    )
    ...
    _, metrics_casa = run_asr_sentence_aldi(
        ...
        aldi_batch_size=args.aldi_batch_size,
        max_length=args.max_text_length,
        progress_desc="Baseline1 [casablanca]",
    )
    ...
    _, metrics_med = run_asr_sentence_aldi(
        ...
        aldi_batch_size=args.aldi_batch_size,
        max_length=args.max_text_length,
        progress_desc="Baseline1 [mediaspeech]",
    )
    ...
    print("[done] baseline1 outputs -> {}".format(args.output_dir))
